...
public class BuildScene2D : EditorWindow
{
    [MenuItem("SRP/Build Scene 2D Renderer")]
    public static void Build()
    {
        // Reimport sprites with correct settings
        ...
        // Camera - simple setup, no UniversalAdditionalCameraData
        ...
        // Use Sprite-Unlit-Default for 2D Renderer compatibility
        var mat = new Material(Shader.Find("Sprites/Default"));
        Debug.Log("[BUILD2D] Using shader: " + (mat.shader != null ? mat.shader.name : "NULL"));

        // Backgrounds
        var bgContainer = new GameObject("BackgroundContainer");
        for (int i = 0; i < 5; i++)
        {
            var s = AssetDatabase.LoadAssetAtPath<Sprite>("Assets/Sprites/backgrounds/bg_0" + (i+1) + ".png");
            ...
            if (s != null)
            {
                ...
                float sw = sh * 16f / 9f;
                go.transform.localScale = new Vector3(sw / s.bounds.size.x, sh / s.bounds.size.y, 1);
            }
            Debug.Log("[BUILD2D] " + go.name + " sprite=" + (s != null ? s.name : "NULL"));
        }
        ...
        Debug.Log("[BUILD2D] ===== Scene built for 2D Renderer =====");
    }
}
